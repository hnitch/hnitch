# Goodreads mood samplers

The profile renderer supports short , review-specific mood samplers without being tied to a hosted model subscription.
...
- The workflow targets only a self-hosted `macOS` / `ARM64` runner carrying the custom `hnitch-processing` label.
...
4. Add the custom runner label `hnitch-processing` and install the runner as a background service.
5. Prevent the Mac Mini from sleeping while connected to power. The display may sleep.
6. Manually run **write Goodreads mood samplers** once and verify the output before enabling its schedule.
...
The repository-level Actions policy should remain limited to GitHub-owned Actions with SHA pinning required. Default workflow token access should remain read-only; this workflow requests `contents: write` explicitly because its final step commits the validated cache and regenerated cards. The `local-processing` environment accepts deployments from protected branches only. `main` blocks force-pushes and deletion while allowing the profile refresh bots to make ordinary commits.
